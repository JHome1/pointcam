...
def main():
    ...
    if args.data_name == 's3dis':
        S3DIS(split='train', data_root=args.data_root, test_area=args.test_area, data_split=args.data_split, cutmix=args.cutmix, select_ratio=args.select_ratio)
        S3DIS(split='val',   data_root=args.data_root, test_area=args.test_area, data_split=args.data_split, cutmix=args.cutmix, select_ratio=args.select_ratio)
    else:
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = S3DIS(split='train', data_root=args.data_root, test_area=args.test_area, voxel_size=args.voxel_size, voxel_max=args.voxel_max, transform=train_transform, shuffle_index=True, loop=args.loop, data_split=args.data_split, cutmix=args.cutmix, select_ratio=args.select_ratio)
    if main_process():
    ...
    if args.evaluate:
        val_transform = None
        val_data = S3DIS(split='val', data_root=args.data_root, test_area=args.test_area, voxel_size=args.voxel_size, voxel_max=800000, transform=val_transform, data_split=args.data_split, cutmix=args.cutmix, select_ratio=args.select_ratio)
        if args.distributed:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, model, criterion, optimizer, epoch, train_data.NEW_LABEL)
        scheduler.step()
        epoch_log = epoch + 1
        if main_process():
...
def train(train_loader, model, criterion, optimizer, epoch, NEW_LABEL):
    ...
    for i, (coord, feat, target, offset) in enumerate(val_loader):
       ...
       #######################################################
        ...
        if args.open_eval == 'msp':
            seg_pred_msp    = F.softmax(output, dim=-1)
            ...
            attent = w1*attent1 + w2*attent2 + w3*attent3
            seg_preds = np.append(seg_preds, (1-attent).cpu().data.numpy())
